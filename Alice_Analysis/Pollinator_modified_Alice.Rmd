...
dat01<-rbind(l2, l3[,-2])
dat01 <-cbind(Type=c(rep("Real", nrow(l2)), rep("Artificial", nrow(l3))), dat01)
dat01
```
#Box 2.
...
SumStats_behavior_time<-dat01 %>% pivot_longer(colnames(dat01[5:ncol(dat01)]), names_to="Insect", values_to="Observations") %>% group_by(Time, Behavior) %>% summarise(Average=mean(Observations), SE=se(Observations))

SumStats_behavior_time

#behaviors by flower type (real or artificial)
SumStats_behavior_flower<-dat01 %>% pivot_longer(colnames(dat01[5:ncol(dat01)]), names_to="Insect", values_to="Observations") %>% group_by(Flower, Behavior) %>% summarise(Average=mean(Observations), SE=se(Observations))

SumStats_behavior_flower

#type of insect by time of day
SumStats_taxa_time<-dat01 %>% pivot_longer(colnames(dat01[5:ncol(dat01)]), names_to="Insect", values_to="Observations") %>% group_by(Time, Insect) %>% summarise(Average=mean(Observations), SE=se(Observations))

SumStats_taxa_time

SumStats_taxa_flower<-dat01 %>% pivot_longer(colnames(dat01[5:ncol(dat01)]), names_to="Insect", values_to="Observations") %>% group_by(Flower, Insect) %>% summarise(Average=mean(Observations), SE=se(Observations))
