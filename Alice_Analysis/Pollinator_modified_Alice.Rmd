...
SumStats_behavior_time<-dat01 %>% pivot_longer(colnames(dat[5:ncol(dat)]), names_to="Insect", values_to="Observations") %>% group_by(Time, Behavior) %>% summarise(Average=mean(Observations), SE=se(Observations))

SumStats_behavior_time

#behaviors by flower type (real or artificial)
SumStats_behavior_flower<-dat01 %>% pivot_longer(colnames(dat[5:ncol(dat)]), names_to="Insect", values_to="Observations") %>% group_by(Flower, Behavior) %>% summarise(Average=mean(Observations), SE=se(Observations))

SumStats_behavior_flower

#type of insect by time of day
SumStats_taxa_time<-dat01 %>% pivot_longer(colnames(dat[5:ncol(dat)]), names_to="Insect", values_to="Observations") %>% group_by(Time, Insect) %>% summarise(Average=mean(Observations), SE=se(Observations))

SumStats_taxa_time

SumStats_taxa_flower<-dat01 %>% pivot_longer(colnames(dat[5:ncol(dat)]), names_to="Insect", values_to="Observations") %>% group_by(Flower, Insect) %>% summarise(Average=mean(Observations), SE=se(Observations))
...
SumStats_behavior<-dat01 %>% pivot_longer(colnames(dat[6:ncol(dat)]), names_to="Insect", values_to="Observations") %>% group_by(Flower, Time, Behavior) %>% summarise(Average=mean(Observations), SE=se(Observations))

SumStats_taxa<-dat01 %>% pivot_longer(colnames(dat[6:ncol(dat)]), names_to="Insect", values_to="Observations") %>% group_by(Flower, Time, Insect) %>% summarise(Average=mean(Observations), SE=se(Observations))

taxa<-colnames(dat01[6:ncol(dat01)])
beh_lower_datyime<-SumStats_behavior$Average-SumStats_behavior$SE
